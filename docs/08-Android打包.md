...
**"本来就放得下"时这段是恒等变换**,所以桌面 1:1 弹层几何一个像素都不动。桌面侧已实测证明(离屏 1100x750 可用区,用共享入口的 `SXCL_ANIM_TRACE` 抓五个弹层矩形):

```
settings#2   rect=(583,462 211x137)   right=794 bottom=599   clamp_fires=False
settings#3   rect=(684,93  110x236)   right=794 bottom=329   clamp_fires=False
versions#0   rect=(684,111 110x170)   right=794 bottom=281   clamp_fires=False
keymap#0     rect=(104,94  180x203)   right=284 bottom=297   clamp_fires=False
keymap#1     rect=(270,144 114x104)   right=384 bottom=248   clamp_fires=False
```

四条钳制条件对五个弹层全是 false ⇒ 桌面几何在数学上不可能被这段改动影响。
(逐像素 A/B 抓图另做:2026-09-21 那次因 `src/ui` 同时在改而作废,须在新基线上重抓 before。)
...
**同一控件的 before/after 直接证据**(36 轮探测:3 轮 × tallmenu 0/20 × 索引 0..5;6 个可见下拉的顺序每次启动都不同,所以要多轮扫):

```
修复前(同一尺寸弹层,两次独立实测): rect=(1034,677 135x203)  bottom=880  可用区底 799  => 超出 81 px
修复后(36 轮里 4 次命中同一个锚点滚出视口的弹层):
  round=1 tm=0  idx=0  rect=(1034,573 135x203)  bottom=776   <== 钳制生效
  round=1 tm=0  idx=2  rect=(1034,573 135x203)  bottom=776
  round=1 tm=20 idx=4  rect=(1034,573 135x203)  bottom=776
  round=1 tm=20 idx=5  rect=(1034,573 135x203)  bottom=776
```

`135x203` 是同一个弹层:y 677→573、底边 880→**776(= 可用区底)**;没触发钳制的弹层底边是 454/719。
⇒ "钳制把出屏的那一例拉回可用区内"是**直接观测到的**,不是推理。
