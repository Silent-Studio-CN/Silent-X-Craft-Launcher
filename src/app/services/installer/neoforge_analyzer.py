...
_OFFICIAL_NEOFORGE = "https://maven.neoforged.net/"


def _mirror_url(url: str) -> str:
    """Replace official URLs with BMCLAPI mirror when configured."""
    from src.core.mirror import maybe_mirror_url
    return maybe_mirror_url(url)


class NeoForgeAnalyzer(LoaderAnalyzer):
    """NeoForge 安装器分析器"""
    ...
    def get_libraries(self) -> List[Dict[str, str]]:
        ...
        for processor in install.get('processors', []):
            for cp_entry in processor.get('classpath', []):
                libraries.append({
                    'name': cp_entry,
                    'url': _mirror_url(_OFFICIAL_NEOFORGE),
